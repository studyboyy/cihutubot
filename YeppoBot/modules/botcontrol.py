...
from YeppoBot.modules.sql_helper.globals import gvarstatus
from YeppoBot.yeppo import _format, asst_cmd, callback, reply_id
    ...
    if event.query.user_id == OWNER_ID:
        await tgbot.send_message(
            ...
            buttons=[
                ...
                (Button.inline("ᴘʟᴜɢɪɴ", data="plugin"),),
                (
...
async def bot_start(event):
    ...
    if chat.id != OWNER_ID:
        ...
        start_msg = f"""
            \n**⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀ 🤖 ᴡᴇʟᴄᴏᴍᴇ ᴛᴏ ᴄɪʜᴜᴛ ʙᴏᴛ 🤖 ⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀**
            \n**Halo [{OWNER}](tg://user?id={OWNER_ID})**
            \n**Apa ada yang bisa saya bantu?**"""
        buttons = [
            (Button.inline("sᴇᴛᴛɪɴɢs ᴠᴀʀ", data="apiset"),),
            (Button.inline("ᴘʟᴜɢɪɴ", data="plugin"),),
