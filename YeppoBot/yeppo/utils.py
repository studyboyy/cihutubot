...
async def autopilot():
    ...
    try:
        ...
        LOGS.info(
            "Channel dan Group Anda terlalu banyak, Hapus Salah Satu Dan Restart Lagi"
        )
        exit(1)
    except BaseException:
        LOGS.info(
...
async def autobot():
    ...
    if isdone.startswith("Sorry,"):
        ran = randint(1, 100)
        username = f"Yeppo{str(who.id)[6:]}{ran}bot"
        await bot.send_message(bf, username)
        await asyncio.sleep(1)
...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"YeppoBot/modules/{shortname}.py")
        name = f"YeppoBot.modules.{shortname}"
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules[f"YeppoBot.modules.{shortname}"] = mod
        LOGS.info(f"Successfully imported {shortname}")
...
def start_assistant(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"YeppoBot/modules/assistant/{shortname}.py")
        name = f"YeppoBot.modules.assistant.{shortname}"
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        ...
        mod.tgbot = bot.tgbot
        spec.loader.exec_module(mod)
        sys.modules[f"YeppoBot.modules.assistant{shortname}"] = mod
